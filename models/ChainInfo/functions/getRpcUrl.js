...
const getRpcUrl = (identifier, callback) => {
    fetch(`https://raw.githubusercontent.com/cosmos/chain-registry/master/${identifier}/chain.json`)
      ...
      .then(json => {
       ...
       // console.log(rpc_api_list);
        if (!rpc_api_list)
          return callback('1document_not_found_no_log', null);
      ...
      .catch(_ => {
        return callback('2document_not_found_no_log', null);
      });
  };
  ...
  const CheckRpcUrl = (rpcUrltry, callback) => {
    fetch(rpcUrltry)
      .then((res) => {
        if (!res.ok)
          return callback('3document_not_found_no_log', null);

        callback(null, rpcUrltry);
      })
      .catch(_ => {
        callback('4document_not_found_no_log', null);
      });
  }
/* 
